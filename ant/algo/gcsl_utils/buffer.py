...
class ReplayBuffer:
    ...
    def reset(self):
        self._actions = np.zeros(
            (self.max_buffer_size, self.max_trajectory_length, *self.env.action_space.shape),
            dtype=self.env.action_space.dtype
        )
        self._states = np.zeros(
            (self.max_buffer_size, self.max_trajectory_length, *self.env.state_space.shape),
            dtype=self.env.state_space.dtype
        )
        self._desired_states = np.zeros(
            (self.max_buffer_size, *self.env.state_space.shape),
            dtype=self.env.state_space.dtype
        )

        internal_goal_shape = self.env._extract_sgoal(self.env.sample_goal()).shape
        self._internal_goals = np.zeros(
            (self.max_buffer_size, self.max_trajectory_length, *internal_goal_shape),
