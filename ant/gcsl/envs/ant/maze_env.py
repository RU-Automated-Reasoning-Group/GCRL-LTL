...
from gcsl.envs.ant import maze_env_utils
...
class MazeEnv(gym.Env):
    ...
    def __init__(
        ...
        if maze_id in ['antfourrooms']:
            ant_xml_file = "ant_for_hac.xml"
        elif maze_id in ['antlongumaze', 'antsmaze', 'antpimaze', 'antomegamaze']:
            ant_xml_file = "ant_for_ris.xml"
        else:
            ant_xml_file = model_cls.FILE

        xml_path = os.path.join(MODEL_DIR, ant_xml_file)
        ...
        if self.elevated:
            ...
            torso.set('pos', '0 0 %.2f' % (0.75 + height_offset))  # [0, 0, 4.75]
        if self.blocks:
    ...
    # [for assign different start position by MWS]
    def reset_with_init_pos_at(self, robot_maze_pos):
        self.t = 0
        self.trajectory = []
        self.wrapped_env.reset()
        xy = (robot_maze_pos[0], robot_maze_pos[1])
        self.wrapped_env.set_xy(xy)
        return self._get_obs()
