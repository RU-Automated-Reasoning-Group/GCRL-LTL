...
class DiscretizedActionEnv(ProxyEnv):
    ...
    def step(self, actionidx):
        return self.wrapped_env.step(self.base_actions[actionidx])

    def step_for_RRT(self, action):
        return self.wrapped_env.step(action)

    def get_action_for_RRT(self, actionidx):
        return self.base_actions[actionidx]

    def get_actionidx_by_action(self, action):
        # choose the actionidx that matchs the action the best(same direction)
        # action_T = np.array(action).T
        # martix = self.base_actions

        action = np.array(action)
        best_cos = -1
        best_index = -1
        b = np.linalg.norm(action)
        for index in range(len(self.base_actions)):
            a = np.linalg.norm(self.base_actions[index])
            if a == 0 or b == 0:
                continue
            else:
                cur_cos_a = np.dot(self.base_actions[index], action) / b
                # cur_cos = np.dot(self.base_actions[index], action)/(a * b)
            if cur_cos_a > best_cos:
                best_cos = cur_cos_a
                best_index = index
