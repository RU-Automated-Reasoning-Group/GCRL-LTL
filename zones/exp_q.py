...
from algo import path_finding, reaching_q
...
def exp(config):
    ...
    model = GCPPO.load(path='./ppo_123_model.zip', device=device)
    q_net = GCPPO.load_q_net(path='./ppo_123_q_net.zip', device=device)
    model = (model, q_net)
    ...
    with torch.no_grad():
        ...
        for i in range(config.eval_repeats):
            ...
            task_info = reaching_q(env, model, GOALS, AVOID_ZONES, value_threshold=config.value_threshold, device=device)
